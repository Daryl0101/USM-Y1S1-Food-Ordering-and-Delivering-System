...
//Basic Functions
void greetings(); //Outputs greeting page to user
...
int main()
{
	...
	//Determine if the identity of the user is a manager or a customer
	do
	{
		cout << endl
			 << setw(64) << "PROFILE\n"
...
void greetings()
{
	...
	cout << endl
		 << setw(71) << "~WELCOME TO TAI~\n"
		 << setw(86) << "~WE SERVE THE BEST HALAL THAI CUISINE IN TOWN~\n"
...
void add_menu(string food_name[], double food_price[], int food_number[], int& low_boundary, int& high_boundary, int food_stock[])
{
	...
	do
	{
		...
		for(int i=low_boundary; i<high_boundary; i++)
		{
			...
			cin >> food_price[i];
			//Check the validity of the price, which must be greater than RM 0
			while(food_price[i]<=0)
			{
				cout << "Please enter a price that is greater than RM 0!: ";
				cin >> food_price[i];
			}
...
void customer_choice(string food_name[], double food_price[], int food_amount_chosen[], int food_amount_chosen_total[], int food_number[], int& total_customer, double& current_sales, double& total_sales, int food_stock[])
{
	...
	cout << "Hello " << customer_name << "!\n"
		 << "Welcome to TAI Thai Food Restaurant!\n"
...
void display_current_order(string food_name[], double food_price[], int food_amount_chosen[], int food_number[], int food_stock[])
{
	cout << endl
		 << setw(21) << "FOOD CART\n"
		 << "-----------------------------------------------------------------------\n"
		 << "Food\t\t\tNo.\tQuantity\tStock\tPrice(RM)\n"
		 << "-----------------------------------------------------------------------";
...
void delivery_choice()
{
	...
	cout
	 ...
	 << "---------------------------------------------\n"
	 << "A." << setw(16) << "Batu Lanchang\n"
	 << "B." << setw(14) << "Sungai Batu\n"
	 << "C." << setw(17) << "Batu Ferringhi\n"
	 << "D." << setw(17) << "Seberang Perai\n"
	 << "---------------------------------------------\n"
	 << "Please choose (A/B/C/D): ";
	cin >> delivery_choice;
	...
	switch(delivery_choice)
	{
		case('A'):
	 	case('a'):
	 			cout << "Batu Lanchang\t\t" << DISTANCE_BL << " KM\t\t" << DISTANCE_BL*DELIVERY_TIME << " minutes\n";
	 			break;
		case('B'):
	 	case('b'):
	 			cout << "Sungai Batu\t\t" << DISTANCE_SB << " KM\t\t" << DISTANCE_SB*DELIVERY_TIME << " minutes\n";
	 			break;
		case('C'):
	 	case('c'):
	 			cout << "Batu Ferringhi\t\t" << DISTANCE_BF << " KM\t\t" << DISTANCE_BF*DELIVERY_TIME << " minutes\n";
	 			break;
		case('D'):
	 	case('d'):
	 			cout << "Seberang Perai\t\t" << DISTANCE_SP << " KM\t\t" << DISTANCE_SP*DELIVERY_TIME << " minutes\n";
	 			break;
	}
	cout << "-----------------------------------------------------------------\n";
}
...
void make_payment(string food_name[], double food_price[], int food_amount_chosen[], int food_number[], double& current_sales, int food_stock[])
{
	...
	cout << fixed << setprecision(2) << endl
		 << setw(22) << "SUBTOTAL\n"
		 << "---------------------------------------------\n"
		 << "Total\t\tRM " << current_sales << endl
		 << "Tax(" << TAX << "%)\tRM " << current_sales*(TAX/100) << endl
		 << "---------------------------------------------\n"
		 << "Subtotal\tRM " << payment_total;
	payment_method_customer(); //Asks customer for a payment method
	//Asks customer to pay the sufficient amount
	cout << fixed << setprecision(2)
		 << "Please input a sufficient amount (RM " << payment_total << "): RM ";
	cin.ignore();
	cin >> customer_payment;
	while(!(customer_payment>=payment_total))
	{
		cout << "Please pay the sufficient amount (RM " << payment_total << "): RM ";
		cin >> customer_payment;
	}
	system("cls");
	...
	cout << "You have successfully made payment (RM " << payment_total << ")\n";
	cout << "Payment: RM " << customer_payment << endl;
	remainder = customer_payment - payment_total; //Calculates the balance of customer
	cout << "Balance: RM " << remainder << endl;
